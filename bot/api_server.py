...
API Endpoints:
    ...
    GET  /api/btc      — BTC hourly price farming data (v2.2)
        ...
        if path.startswith('/api/'):
            ...
            if api_path == '/api/status':
                ...
                try:
                    ...
                    self.send_json({"error": str(e)}, 500)
            elif api_path == '/api/btc':
                try:
                    self.send_json(get_btc_data())
                except Exception as e:
                    self.send_json({"error": str(e)}, 500)
...
def get_sports_data():
    ...
    # Step 1: Get live scoreboard from ESPN (NBA + NCAAB)
    espn_leagues = {
        "nba": "https://site.api.espn.com/apis/site/v2/sports/basketball/nba/scoreboard",
        "ncaab": "https://site.api.espn.com/apis/site/v2/sports/basketball/mens-college-basketball/scoreboard",
    }
    events = []
    league_map = {}  # event_id -> league
    for league_key, espn_url in espn_leagues.items():
        try:
            ...
            for ev in scoreboard.get("events", []):
                league_map[ev.get("id", "")] = league_key
                events.append(ev)
        except Exception as e:
            result[f"espn_{league_key}_error"] = str(e)
    ...
    for event in events:
        try:
            ...
            game_data = {
                ...
                "league": league_map.get(event.get("id", ""), "nba"),
            }
    ...
    for series, key in [("KXNBASPREAD", "spread"), ("KXNBATOTAL", "total"), ("KXNCAAMBSPREAD", "spread"), ("KXNCAAMBTOTAL", "total")]:
        try:
...
def get_btc_data():
    """Fetch BTC price + KXBTCD market data for the dashboard."""
    import math
    ...
    try:
        ...
        result["btc_price"] = float(data["data"]["amount"])
    except Exception as e:
        result["btc_price_error"] = str(e)

    btc_price = result["btc_price"]
    if not btc_price:
        return result

    # 2. Fetch KXBTCD markets
    try:
        url = f"{KALSHI_PUBLIC_API}/markets?series_ticker=KXBTCD&status=open&limit=200"
        data = _kalshi_public_get(url)
        all_markets = data.get("markets", [])
    except Exception:
        all_markets = []

    # 3. Process markets — find ones near the current price
    ...
    for m in all_markets:
        ...
        try:
            ...
            hours_to_settle = (close_dt - now_utc).total_seconds() / 3600.0
        except Exception:
            hours_to_settle = -1

        if hours_to_settle < 0.05 or hours_to_settle > 48:
            continue

        distance = btc_price - strike

        # Only include markets within $8k of current price
        if abs(distance) > 8000:
            continue

        # Simple log-normal model probability
        annual_vol = 0.50
        T = hours_to_settle / (365.25 * 24)
        sigma = annual_vol
        if T > 0 and strike > 0 and btc_price > 0:
            d = (math.log(btc_price / strike) + 0.5 * sigma * sigma * T) / (sigma * math.sqrt(T))
    ...
    result["markets"].sort(key=lambda x: abs(x["distance"]))
    result["market_count"] = len(result["markets"])
